module Enterprise::Whatsapp::Providers::Whatsapp360DialogService
  ...
  def send_call_permission_request(to_phone_number, body_text = I18n.t('conversations.messages.whatsapp.call_permission_request_body'))
    response = HTTParty.post(
      "#{api_base_path}/messages", headers: api_headers, body: permission_request_body(to_phone_number, body_text)
    )
  ...
  def initiate_call(to_phone_number, sdp_offer)
    response = HTTParty.post(
      "#{api_base_path}/calling/calls", headers: api_headers, body: initiate_call_body(to_phone_number, sdp_offer)
    )
  ...
  def update_calling_status(status)
    response = HTTParty.post(
      "#{api_base_path}/calling/settings",
      headers: api_headers,
    ...
    message = parsed.dig('error', 'error_user_msg') || parsed.dig('meta', 'developer_message') || 'Failed to update calling status'
  ...
  def call_api(action_name, body)
    url = "#{api_base_path}/calling/calls"
  ...
  def process_initiate_call_response(response)
    ...
    error_msg = parsed.dig('error', 'error_user_msg') || parsed.dig('meta', 'developer_message') || 'Failed to initiate call'
